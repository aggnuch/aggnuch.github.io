...
function gidle() {
	...
	if (inst < 3) {
		...
		document.getElementById("instructionsField3").style.display = "none";
		if (firstTime == "true") {
			show("playingField");
			startTimer();
		}
...
function menuStats() {
	...
	hide("menuField");
	show("statsField");
	display("stats1",overallGames);
	display("stats2",`${Math.floor(overallTime * 1000) / 1000} seg.`);
	display("stats3",overallWords);
	display("stats4",Math.round(overallWords / overallGames * 10)/10);
}
